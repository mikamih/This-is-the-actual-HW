...
var renderProductPickedList = (productPickedList) => {
  console.log("renderProductPickedList", productPickedList.length);
  if (productPickedList.length === 0) {
    document.getElementById("modal-cart-content").innerHTML = `
      <p class="text-center">You don't have anything in here </p>
    `;
  } else {
...
const order = () => {
  if (productPickedList.length === 0) {
    return Swal.fire({
      title: "Error!",
      text: "Oi! there is no item in here",
      icon: "error",
  ...
  const htmlString = productList.reduce((accHTML, product) => {
    ...
    <div class="product card shadow-sm">
      ...
      <div class="card-body">
        ...
        <b class="text-danger">${product.salePrice} RP </b>
